...
namespace BusinessLogicLayer.ValueToProductService
{
    public class ValueToProductLogic
    {
        ...
        public async Task<bool> AddValueToProduct(ValueToProduct model)
        {
            ...
            {
                if (!ValueToProductRepository.Get(ks => ks.Value_Id == model.Value_Id && ks.Product_Id == model.Product_Id).Any())
                {
        ...
        //{
        //    if (ValueToProductRepository.Get(s => s.Value_Id == Id).Any())
        //    {
        ...
        {
            if (ValueToProductRepository.Get(s => s.Value_Id == Value_Id && s.Product_Id == Product_Id).Any())
            {
                var ValueToProduct = ValueToProductRepository.Get(s => s.Product_Id == Product_Id && s.Value_Id == Value_Id).FirstOrDefault();
        ...
        {
            var model = ValueToProductRepository.Get(ks => ks.Value_Id == Value_Id && ks.Product_Id == Product_Id).FirstOrDefault();
            return model;
        }
        public ICollection<ValueToProduct> ValueToProductList()
        {
            ...
            return ValueToProductRepository.Get(null, ks => ks.Product, ks => ks.Value).ToList();
        }
    }
